...
class RoomImageCache(private val imageDao: ImageDao, val dir: File) : IImageCache {
    ...
    override fun saveImage(url: String, bytes: ByteArray) = Completable.create { emitter ->
        ...
        imageDao.insert(RoomCachedImage(url, imageFile.path))
        emitter.onComplete()
    }.subscribeOn(Schedulers.io())

    override fun contains(url: String) = Single.fromCallable { imageDao.findByUrl(url) != null }.subscribeOn(Schedulers.io())
